...
export default class App extends React.Component {
	...
	render() {
		...
		return (
				...
				<View style={{backgroundColor:'yellow',margin:20,flexDirection:'column'}} >
				 
					<View style={{flexDirection:'row',justifyContent:'space-around'}} >
					{this.showValue()}
						<TouchableOpacity onPress={()=>{this.setState({viewIndex: this.state.viewIndex})}} >
							<Text>Bugün</Text>
						</TouchableOpacity>

						<TouchableOpacity onPress={()=>{this.setState({viewIndex: this.state.viewIndex==0})}}> 
							<Text>Geçen Hafta</Text>
						</TouchableOpacity>
	...
	showValue(){
		if (this.state.viewIndex==-1){
			return <Text style={{alignItems:'center'}} >+453.8%</Text>;
			
		}
		else if(this.state.viewIndex==0){
			return <Text>-3.70%</Text>;
			
		}
		else if(this.state.viewIndex==1){
			return <Text>+9.98%</Text>;
			

		}
		else if(this.state.viewIndex==2){
			return <Text>-0.20%</Text>;
